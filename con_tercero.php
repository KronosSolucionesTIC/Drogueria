...
if($eliminacion==1) {//confirmacion de insercion  
	$campos="estado_vendedor ='2'";
	$error=editar("vendedor",$campos,'cod_ven',$eli_codigo); 
	if ($error >=1)
...
if(!empty($busquedas)) { #codigo para buscar 
	...
	$where=" where estado_vendedor = 1 and $busquedas ";
}#codigo para buscar 
else{
	$where=" where estado_vendedor = 1";
}

$sql="SELECT cod_ven, nom_ven, tel_ven, cod_emp_ven, cod_bod_ven , nom_rso,nom_bod FROM vendedor
                    ...
                    <tr>
                      <td bgcolor="#D1D8DE"><table width="624" border="0" cellspacing="0" cellpadding="0">
                        <tr>
                          ...
                          <td width="17"> 
						  <? if ($insertar==1) {?>
					  	  <img src="imagenes/agregar.png" width="16" height="16"  alt="Nuevo Registro" style="cursor:pointer" onClick="location.href='man_tercero.php?codigo=0&editar=<?=$editar?>&insertar=<?=$insertar?>&eliminar=<?=$eliminar?>'"/>
					  	  <? } ?></td>
                    ...
                    <tr>
                      <td><table width="624" border="0"  cellpadding="0">
						...
						while($db->next_row()) {
							...
							if ($editar==1)
							 	echo "<img src='imagenes/icoeditar.png' alt='Editar Registro' width='16' height='16' style='cursor:pointer'  onclick='document.forma_$db->cod_ven.submit()'/></td>";
							else 
								echo "<img src='imagenes/e_icoeditar.png' width='16' height='16'  /></td>";
                            echo 	"<td align='center'>";
							if ($eliminar==1)
								echo"<img src='imagenes/icoeliminar.png' width='16' alt='Eliminar Registro' height='16' style='cursor:pointer' onclick=confirmar($db->cod_ven) /></td> ";
							else
