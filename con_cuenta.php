...
if($eliminacion==1) {//confirmacion de insercion  
	$campos="estado_cuenta ='2'";
	$error=editar("cuenta",$campos,'cod_cuenta',$eli_codigo); 
	if ($error >=1)
...
if(!empty($busquedas)) { #codigo para buscar 
	...
	$where="where $busquedas and estado_cuenta = 1";
}#codigo para buscar 
else{
	$where="where estado_cuenta = 1";
}

$sql="SELECT * FROM cuenta $where ORDER BY cod_contable";
...
<form id="forma_total" name="forma_total" method="post" action="man_cuenta.php">
                  <table width="624" border="0" cellspacing="0" cellpadding="0" align="center" >
                    <tr>
                      <td bgcolor="#D1D8DE"><table width="624" border="0" cellspacing="0" cellpadding="0">
                        <tr>
                          ...
                          <td width="19"> 
						  <? if ($insertar==1) {?>
					  	  <img src="imagenes/agregar.png" width="16" height="16"  alt="Nuevo Registro" style="cursor:pointer" onClick="location.href='man_cuenta.php?codigo=0&editar=<?=$editar?>&insertar=<?=$insertar?>&eliminar=<?=$eliminar?>'"/>
					  	  <? } ?></td>
                          ...
                          <td width="160" valign="middle"><select name="campos" class="textfieldlista" id="campos" >
                            ...
                            <option value="cod_contable">Codigo</option>
                            <option value="desc_cuenta">Nombre</option>
                    ...
                    <tr>
                      <td><table width="628" border="0"  cellpadding="0">
                        <tr>
                          <td width="120" class="ctablasup">COD CONTABLE</td>
						  <td width="450" class="ctablasup">NOMBRE CUENTA</td>
						  <td  class="ctablasup"  width="46">NIVEL</td>
                          <td  class="ctablasup"  width="142">OPCIONES</td>
                        </tr>
						<? 
						
						echo "<tr style='display:none'><td ><form name='forma_0' action='man_cuenta.php'>";
						echo "  </form> </td></tr>  ";						  
						$estilo="ctablablanc";
						$estilo="ctablagris";
						...
						while($db->next_row()) {
							...
							echo "<tr class='$estilo' $cambio_celda> <form name='forma_$db->cod_cuenta' action='man_cuenta.php'>  ";
							echo "<td>$db->cod_contable</td>";
							echo "<td>$db->desc_cuenta</td>";
							echo "<td>$db->nivel</td>";
                            echo "<td aling='center' >"; 
							echo 	"<table width='100%' border='0' cellspacing='0' cellpadding='0'>";
                            echo 	" <tr>  <td align='center'> <input type='hidden' name='codigo' value='$db->cod_cuenta'>";
							if ($editar==1)
							 	echo "<img src='imagenes/icoeditar.png' alt='Editar Registro' width='16' height='16' style='cursor:pointer'  onclick='document.forma_$db->cod_cuenta.submit()'/></td>";
							else 
								echo "<img src='imagenes/e_icoeditar.png' width='16' height='16'  /></td>";
                            echo 	"<td align='center'>";
							if ($eliminar==1)
								echo"<img src='imagenes/icoeliminar.png' width='16' alt='Eliminar Registro' height='16' style='cursor:pointer' onclick=confirmar($db->cod_cuenta) /></td> ";
							else
...
<form name="forma" method="post" action="con_cuenta.php">
