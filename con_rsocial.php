...
if($eliminacion==1) {//confirmacion de insercion  
	$campos="estado_rsocial='2'";
	$error=editar("rsocial",$campos,'cod_rso',$eli_codigo); 
	if ($error >=1)
...
$sql="select * FROM rsocial where estado_rsocial = 1 $where";
...
<tr>
<td valign="top" >
<form id="forma_total" name="forma_total" method="post" action="man_rsocial.php">
                  <table width="624" border="0" cellspacing="0" cellpadding="0" align="center" >
                    <tr>
                      <td bgcolor="#D1D8DE"><table width="624" border="0" cellspacing="0" cellpadding="0">
                        <tr>
                          ...
                          <td width="19"> 
						  <? if ($insertar==1) {?>
					  	  <img src="imagenes/agregar.png" width="16" height="16"  alt="Nuevo Registro" style="cursor:pointer" onClick="location.href='man_rsocial.php?codigo=0&editar=<?=$editar?>&insertar=<?=$insertar?>&eliminar=<?=$eliminar?>'"/>
					  	  <? } ?></td>
                          ...
                          <td width="160" valign="middle"><select name="campos" class="textfieldlista" id="campos" >
                            ...
                            <option value="nom_rso">Nombre</option>
							<option value="nit_rso">Nit</option>
                    ...
                    <tr>
                      <td><table width="624" border="0"  cellpadding="0">
						...
						echo "<tr style='display:none'><td ><form name='forma_0' action='man_rsocial.php'>";
						echo "  </form> </td></tr>  ";						  
						$estilo="ctablablanc";
						$estilo="ctablagris";
						...
						while($db->next_row()) {
							...
							echo "<tr class='$estilo' $cambio_celda> <form name='forma_$db->cod_rso' action='man_rsocial.php'>  ";
							echo "<td >$db->nom_rso </td>";
							echo "<td >$db->nit_rso </td>";
                          	
							
                            echo "<td aling='center' >"; 
							echo 	"<table width='100%' border='0' cellspacing='0' cellpadding='0'>";
                            echo 	" <tr>  <td align='center'> <input type='hidden' name='codigo' value='$db->cod_rso'>";
							if ($editar==1)
							 	echo "<img src='imagenes/icoeditar.png' alt='Editar Registro' width='16' height='16' style='cursor:pointer'  onclick='document.forma_$db->cod_rso.submit()'/></td>";
							else 
								echo "<img src='imagenes/e_icoeditar.png' width='16' height='16'  /></td>";
                            echo 	"<td align='center'>";
							if ($eliminar==1)
								echo"<img src='imagenes/icoeliminar.png' width='16' alt='Eliminar Registro' height='16' style='cursor:pointer' onclick=confirmar($db->cod_rso) /></td> ";
							else
...
<form name="forma" method="post" action="con_rsocial.php">
