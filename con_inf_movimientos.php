...
function MM_jumpMenu(targ,selObj,restore){ //v3.0
  ...
  if (restore) selObj.selectedIndex=0;
	...
	function abrir() {	
		...
		else if(document.getElementById('fec_fin').value < document.getElementById('fec_ini').value ){
		 	alert('La fecha final no puede ser mejor')
		 }
		else {
		 	var fec_ini = document.getElementById('fec_ini').value;
			var fec_fin = document.getElementById('fec_fin').value;
			var pro = document.getElementById('proveedor').value;
			
			imprimir_inf("inf_mov.php",'0&fec_ini='+fec_ini+"&fec_fin="+fec_fin+"&pro="+pro,'mediano');
