...
if($eliminacion==1) {//confirmacion de insercion  
	$error=eliminar("pedido",$eli_codigo,"cod_ped");
	if ($error >=1)
...
$where=" where $busquedas   and cod_proy_ped=$global[0] ";
...
if(!empty($busquedas)) { #codigo para buscar 
	...
	$where=" where  cod_proy_ped=$global[0] ";


$cantidad_paginas=paginar('SELECT  cod_ped, fec_ped , cod_alm_ped , pri_ped, fec_ent_ped ,  
							cod_pro_ped ,cod_proy_ped , obs_ped, cod_usu_ped ,nom_alm ,proveedor.nom_pro ,
							proyecto.nom_pro  ,concat(usuario.nom_usu,concat("  ",usuario.ape_usu)) AS usuario FROM pedido  INNER JOIN  almacen  ON almacen.cod_alm=pedido.cod_alm_ped  				INNER JOIN proveedor ON proveedor.cod_pro=pedido.cod_pro_ped INNER JOIN proyecto ON proyecto.cod_pro=pedido.cod_proy_ped INNER JOIN usuario ON usuario.cod_usu=cod_usu_ped '.$where.'' );
...
<tr>
<td valign="top" >
<form id="forma_total" name="forma_total" method="post" action="man_pedido.php">
                  <table width="624" border="0" cellspacing="0" cellpadding="0" align="center" >
                    <tr>
                      <td bgcolor="#D1D8DE"><table width="624" border="0" cellspacing="0" cellpadding="0">
                        <tr>
                          ...
                          <td width="19"> 
						  <? if ($insertar==1) {?>
					  	  <img src="imagenes/agregar.png" width="16" height="16"  alt="Nuevo Registro" style="cursor:pointer" onClick="location.href='man_pedido.php?codigo=0&editar=<?=$editar?>&insertar=<?=$insertar?>&eliminar=<?=$eliminar?>'"/>
					  	  <? } ?></td>
                          ...
                          <td width="160" valign="middle"><select name="campos" class="textfieldlista" id="campos" >
                            ...
                            <option value="cod_ped">Numero</option>
							<option value="fec_ped">Fecha</option>
							<option value="nom_alm">Almacen</option>
							<option value="pri_ped">Prioridad</option>
                    ...
                    <tr>
                      <td><table width="624" border="0"  cellpadding="0">
						...
						echo "<tr style='display:none'><td ><form name='forma_0' action='man_pedido.php'>";
						echo "  </form> </td></tr>  ";						  
						$estilo="ctablablanc";
						$estilo="ctablagris";
						$sql='SELECT  cod_ped, fec_ped , cod_alm_ped , pri_ped, fec_ent_ped ,  cod_pro_ped ,cod_proy_ped , obs_ped, cod_usu_ped ,nom_alm ,proveedor.nom_pro ,
							...
							INNER JOIN proveedor ON proveedor.cod_pro=pedido.cod_pro_ped INNER JOIN proyecto ON proyecto.cod_pro=pedido.cod_proy_ped INNER JOIN usuario ON usuario.cod_usu=cod_usu_ped '.$where.' order by cod_ped desc '.$paginar ; 
						//echo $sql;
						...
						while($db->next_row()) {
							...
							if ($editar==1)
							 	echo "<img src='imagenes/icoeditar.png' alt='Editar Registro' width='16' height='16' style='cursor:pointer'  onclick='document.forma_$db->cod_ped.submit()'/></td>";
							else 
								echo "<img src='imagenes/e_icoeditar.png' width='16' height='16'  /></td>";
                            echo 	"<td align='center'>";
							if ($eliminar==1)
								echo"<img src='imagenes/icoeliminar.png' width='16' alt='Eliminar Registro' height='16' style='cursor:pointer' onclick=confirmar($db->cod_ped) /></td> ";
							else
								echo"<img src='imagenes/e_icoeliminar.png' width='16' height='16'  /></td> ";
							
							echo" <td align='center'> <img src='imagenes/imprimir.png' width='16' alt='Eliminar Registro' height='16' style='cursor:pointer'  onclick=imprimir($global[2],$db->cod_ped,'imp_pedido.php') /></td> ";
                            echo "  </tr> </table>  </td>  ";
...
<form name="forma" method="post" action="con_pedido.php"><input type="hidden" name="editar" id="editar" value="<?=$editar?>">
