...
function agregar_ruta(){
    ...
    var cod_ruta = document.getElementById("rutas").options[document.getElementById("rutas").selectedIndex].value;
	if (cod_ruta=='0'){
		agregar_ruta_masivo();
	}
	else if (cod_ruta!=''){    
	    if (verifica_rep_emp(cod_ruta) != 1) {
			var nombre = document.getElementById("rutas").options[document.getElementById("rutas").selectedIndex].text;
			var combo=document.getElementById("combo_empresa_consulta");
			var num = document.getElementById('cantidadEmp');	
			num.value = parseInt(num.value) + 1;
			document.getElementById('arreglo_empresa').value += cod_ruta+',';
			if (cod_ruta > 0){
				combo.options[combo.options.length] = new Option(nombre,cod_ruta);
			}
		}
	}
}

function verifica_rep_emp(cod_ruta)
{
	...
	if (codigos != '') {
	    codigos = codigos.split(",");
		for(g=0; g<=codigos.length; g++){  
			if (cod_ruta===codigos[g]) { 
				alert ("Ya se agregó esta ruta "+codigos[g]);
				repetido=1;
			}
		}
	}
...
function agregar_ruta_masivo() {
	var combo=document.getElementById("combo_empresa_consulta");
	combo.options.length=0;
	var cant=0;
	var cod_ruta=0;
	var num = document.getElementById('cantidadEmp');	
	document.getElementById('tipo_rutas').value = 1;
	 	
	<? 
		$db = new Database();	
		$sql="SELECT * from ruta WHERE estado_ruta = 1";
		$db->query($sql);
		while($db->next_row()){
		$anadelo = true;

		 if ($anadelo) {
			 echo "combo.options[cant] = new Option(\"$db->desc_ruta\",'$db->cod_ruta');  \n";
			 echo  "cant++;  \n 	";
			 echo "cod_ruta = '$db->cod_ruta';";
			 echo "	document.getElementById('arreglo_empresa').value=document.getElementById('arreglo_empresa').value+cod_ruta+',';";
		  }
		}
	?>	
	num.value = combo.options.length + 1;
...
function abrir() {		
	...
	else 
	{
		...
		var tipo_rutas = document.getElementById('tipo_rutas').value;
		imprimir_inf("inf_ventas_ruta.php",'0&fechas='+fechas+'&fechas2='+fechas2+'&combo_ciudad='+arreglo_ciudad+'&val_inicial='+val_inicial+'&tipo_rutas='+tipo_rutas+'&combo_empresa='+arreglo_empresa,'mediano');
	}
}
        ...
        <table width="710" border="0" cellspacing="0" cellpadding="0" align="center" >
          <tr>
            <td width="710" bgcolor="#D1D8DE"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                <tr>
                  <td height="27" colspan="7"><div align="center" class="subfongris">INFORME DE VENTAS POR RUTAS</div></td>
                </tr>
                <tr>
                ...
                <tr>
                  <td height="20"></td>
                  <td class="ctablaform">Ruta</td>
                  <td colspan="1"><? combo_evento_where("rutas","ruta","cod_ruta","desc_ruta","",""," where estado_ruta = 1"); ?></td>
                  <td width="125"><span class="textorojo">*
                    <input name="Submit" type="button" class="botones" onClick="agregar_ruta()" value="Agregar ruta">
                    </span> </td>
                ...
                <tr>
                  ...
                  <td colspan="6"><p align="center">
                    ...
                    <p align="center">
                      <input name="Submit2" type="button" class="botones" onClick="borrar_empresa()" value="Borrar ruta">
