...
if($eliminacion==1) {//confirmacion de insercion  
	$campos="estado_m_traslado ='2'";
	$error=editar("m_traslado",$campos,'cod_tras',$eli_codigo);					
	if ($error >=1)
...
if(!empty($busquedas)) { #codigo para buscar 
	...
	$where="where estado_m_traslado = 1 and $busquedas ";
}#codigo para buscar 
else{
	$where=" where estado_m_traslado = 1";
}
	 
	 
	$sql=" SELECT 
  m_traslado.cod_tras,
  m_traslado.cod_bod_sal_tras,
  m_traslado.cod_bod_ent_tras,
  m_traslado.fec_tras,
  m_traslado.cod_usu_tras,
  bodega.cod_bod,
  bodega.nom_bod AS bodega_salida,
  bodega1.cod_bod,
  bodega1.nom_bod AS bodega_entrada
FROM
  m_traslado
  INNER JOIN bodega ON (m_traslado.cod_bod_sal_tras = bodega.cod_bod)
  INNER JOIN bodega bodega1 ON (m_traslado.cod_bod_ent_tras = bodega1.cod_bod)
   $where  and $where_cli ORDER BY cod_tras DESC";
...
<tr>
<td valign="top" >
<form id="forma_total" name="forma_total" method="post" action="man_traslados.php">
                    ...
                    <tr>
                      <td bgcolor="#D1D8DE"><table width="624" border="0" cellspacing="0" cellpadding="0">
                        <tr>
                          ...
                          <td width="19"> 
						  <? if ($insertar==1) {?>
					  	  <img src="imagenes/agregar.png" width="16" height="16"  alt="Nuevo Registro" style="cursor:pointer" onClick="location.href='man_d_traslados.php?codigo=0&editar=<?=$editar?>&insertar=<?=$insertar?>&eliminar=<?=$eliminar?>'"/>
					  	  <? } ?></td>
                          ...
                          <td width="160" valign="middle"><select name="campos" class="textfieldlista" id="campos" >
							...
							<option value="fec_tras">Fecha</option>
                            <option value="bodega1.nom_bod">Bodega Entrada</option>
							<option value="bodega.nom_bod">Bodega salida</option>
                    ...
                    <tr>
                      <td><table width="624" border="0"  cellpadding="0">
                        <tr>
						  ...
						  <td  class="ctablasup">BODEGA SALIDA</td>
						...
						echo "<tr style='display:none'><td ><form name='forma_0' action='man_traslados.php'>";
						echo "  </form> </td></tr>  ";						  
						$estilo="ctablablanc";
						$estilo="ctablagris";
						...
						while($db->next_row()) {
							...
							echo "<tr class='$estilo' $cambio_celda> <form name='forma_$db->cod_tras' action='man_traslados.php'>  ";
							echo "<td >$db->cod_tras</td>";
                          	echo "<td >$db->fec_tras</td>";
							echo "<td >$db->bodega_salida </td>";
							echo "<td >$db->bodega_entrada </td>";
							$dbu = new Database();
							$sqlu = "SELECT nom_usu FROM usuario WHERE cod_usu = '$db->cod_usu_tras'";
							$dbu->query($sqlu);
							$dbu->next_row();
							echo "<td >$dbu->nom_usu</td>";
							...
							if ($eliminar==1)
								echo"<img src='imagenes/icoeliminar.png' width='16' alt='Eliminar Registro' height='16' style='cursor:pointer' onclick=confirmar($db->cod_tras) /></td> ";
							else
								echo"<img src='imagenes/e_icoeliminar.png' width='16' height='16'  /></td> ";
								
								
							//impresion	
							echo "<td align='center'><img src='imagenes/mirar.png' width='16' height='16'  style=\"cursor:pointer\" onClick=\"imprimir_inf('inf_traslados.php',$db->cod_tras,'mediano')\" /></td>";	
                            echo "  </tr> </table>  </td>  ";
...
<form name="forma" method="post" action="con_traslados.php">
