...
if($eliminacion==1) {//confirmacion de insercion  
	$campos="estado_punto_venta ='2'";
	$error=editar("punto_venta",$campos,'cod_pun',$eli_codigo); 
	if ($error >=1)
...
if(!empty($busquedas)) { #codigo para buscar 
	...
	$where=" where estado_punto_venta = 1 and $busquedas ";
}#codigo para buscar 
else{
	$where=" where estado_punto_venta = 1";
}

 $sql=" SELECT cod_pun, nom_pun,punto_venta.cod_pun,punto_venta.cod_ven,bodega.nom_bod,usuario.nom_usu,rsocial.nom_rso
FROM punto_venta 
LEFT JOIN bodega ON punto_venta.cod_bod=bodega.cod_bod 
LEFT JOIN usuario on usuario.cod_usu =punto_venta.cod_ven 
LEFT JOIN rsocial on punto_venta.cod_rso=rsocial.cod_rso  $where ";
...
<tr>
<td valign="top" >
<form id="forma_total" name="forma_total" method="post" action="man_puntoventa.php">
                    ...
                    <tr>
                      <td bgcolor="#D1D8DE"><table width="624" border="0" cellspacing="0" cellpadding="0">
                        <tr>
                          ...
                          <td width="19"> 
						  <? if ($insertar==1) {?>
					  	  <img src="imagenes/agregar.png" width="16" height="16"  alt="Nuevo Registro" style="cursor:pointer" onClick="location.href='man_puntoventa.php?codigo=0&editar=<?=$editar?>&insertar=<?=$insertar?>&eliminar=<?=$eliminar?>'"/>
					  	  <? } ?></td>
                          ...
                          <td width="160" valign="middle"><select name="campos" class="textfieldlista" id="campos" >
                            ...
                            <option value="nom_pun">Punto de venta</option>
							<option value="nom_bod">Bodega</option>
							<option value="nom_usu">Responsable</option>
                    ...
                    <tr>
                      <td><table width="624" border="0"  cellpadding="0">
                        <tr>
						  ...
						  <td  class="ctablasup">PUNTO DE VENTA </td>
						  <td  class="ctablasup">BODEGA</td>
						  <td  class="ctablasup"> RESPONSABLE </td>
                          <td  class="ctablasup"  width="112">OPCIONES</td>
                        </tr>
						<? 
						echo "<tr style='display:none'><td ><form name='forma_0' action='man_puntoventa.php'>";
						echo "  </form> </td></tr>  ";						  
						$estilo="ctablablanc";
						$estilo="ctablagris";
						...
						while($db->next_row()) {
							...
							echo "<tr class='$estilo' $cambio_celda> <form name='forma_$db->cod_pun' action='man_puntoventa.php'>  ";
                          	echo "<td >$db->nom_rso</td>";
							echo "<td >$db->nom_pun</td>";
							echo "<td >$db->nom_bod </td>";
							echo "<td >$db->nom_usu </td>";
                            echo "<td aling='center' >"; 
							echo 	"<table width='100%' border='0' cellspacing='0' cellpadding='0'>";
                            echo 	" <tr>  <td align='center'> <input type='hidden' name='codigo' value='$db->cod_pun'>";
							if ($editar==1)
							 	echo "<img src='imagenes/icoeditar.png' alt='Editar Registro' width='16' height='16' style='cursor:pointer'  onclick='document.forma_$db->cod_pun.submit()'/></td>";
							else 
								echo "<img src='imagenes/e_icoeditar.png' width='16' height='16'  /></td>";
                            echo 	"<td align='center'>";
							if ($eliminar==1)
								echo"<img src='imagenes/icoeliminar.png' width='16' alt='Eliminar Registro' height='16' style='cursor:pointer' onclick=confirmar($db->cod_pun) /></td> ";
							else
...
<form name="forma" method="post" action="con_puntoventa.php">
