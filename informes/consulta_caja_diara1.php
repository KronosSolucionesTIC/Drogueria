...
if($guardar_todo==1) {
	...
	$sql = "SELECT 
		 ...
		 WHERE  ( fecha >='$fec_ini' AND fecha <='$fec_fin' ) AND usuario.cod_usu != 33 AND bodega1.cod_bod != 970 AND bodega1.cod_bod != 1252 AND bodega.cod_bod != 363 AND bodega.cod_bod != 367 AND bodega.cod_bod != 370 AND bodega.cod_bod != 376  AND bodega.cod_bod != 340 AND ruta.cod_ruta != 31 AND bodega.cod_bod != 375
...
<table width="731" border="1" cellpadding="2" cellspacing="1"   class="textoproductos1" align="center">
  <tr>
    <td colspan="8" class="ctablasup" align="center">CUENTA CAJA EFFORT</td>
  </tr>
  <tr>
  ...
  <tr>
	...
	?>
  <tr>
    <td colspan="8" class="ctablasup" align="center">CUENTA CAJA BLAUS</td>
  </tr>
  <tr>
    <td colspan="8" align="center"><br></td>
  </tr>
  <tbody colspan="8" id="facturacion2" style="display:none" width="100%">
   <tr>
    <td class="subtitulosproductos">Responsable</td>
    <td width="17%" class="subtitulosproductos">Factura No. </td>
    <td class="subtitulosproductos">Fecha</td>
...
?>
		<tr>
		  <td class="textoproductos1"><?=$db_blaus->responsable?></td>
		  <td class="textoproductos1"><?=$db_blaus->num_fac?></td>
		  <td width="11%" class="textoproductos1"><?=$db_blaus->fecha?></td>
		  <td width="32%" class="textoproductos1"><?=$db_blaus->tipo_pago?></td>
		  <td width="32%" class="textoproductos1"><?=$db_blaus->cliente; ?></td>
		  <td width="10%" colspan="3" class="textoproductos1" align="right"><?=number_format($valor_blaus,0,".",".")?></td>
		</tr>
        		<?
		$cantidad_max_blaus += $valor_blaus;
		$total_fact_1 += $valor_blaus;
		}
	} 	
	?>
  </tbody>
  <tr>
    ...
    <td class="subtitulosproductos" align="right"><b>TOTAL FACTURACION: </b></td>
    <td colspan="3" class="subtitulosproductos" align="right"><b>
      <?=number_format($cantidad_max_blaus,0,".",".")?>
      </b></td>
  </tr>
  <tr>
    <td align="center" colspan="8"><input name="btn_fac" type="button" class="botones1"  id="btn_fac4" onClick="ver_facturacion('facturacion2','btn_fac2')" value="Ver Detalles" /></td>
  </tr>
  <tr>
  ...
  INNER JOIN bodega1 ON bodega1.cod_bod = m_factura.cod_cli
  WHERE(fecha >='$fec_ini' AND fecha <='$fec_fin') AND estado='anulado' AND bodega1.cod_bod_cli = 225";
  $dba->query($sqla);
  	while($dba->next_row()){
...
?>
  ...
  <tr>
	...
	WHERE  ( fec_emi >='$fec_ini' AND fec_emi <='$fec_fin' )  AND tipo_mov = 4 AND nivel = 5 AND estado_mov = 1 AND cod_centro = 1";
	$db_ver->query($sql);
	if ($db_ver->next_row()){ 
...
if($guardar_todo==1) {
	...
	$sql = "SELECT 
		 ...
		 LEFT JOIN usuario ON ( usuario.cod_usu=m_factura.cod_usu)
		 INNER JOIN bodega ON bodega.cod_bod = m_factura.cod_bod 
		 WHERE  ( fecha >='2014-12-04' AND fecha <='$fec_fin' ) AND usuario.cod_usu != 33 AND bodega1.cod_bod != 970 AND bodega1.cod_bod != 1252 AND bodega.cod_bod != 363 AND bodega.cod_bod != 367 AND bodega.cod_bod != 370 AND bodega.cod_bod != 376  AND bodega.cod_bod != 340 AND ruta.cod_ruta != 31 AND bodega.cod_bod != 375
		 AND estado IS  NULL $where_cli  ";
	$db_ver->query($sql);	
	$cantidad_max=0;
?>
...
<table width="731" border="1" cellpadding="2" cellspacing="1"   class="textoproductos1" align="center">
  ...
  <tr>
	...
	INNER JOIN cuenta ON cuenta.cod_cuenta = d_movimientos.cuenta_dmov
	INNER JOIN centro_costos ON centro_costos.cod_centro = d_movimientos.centro_dmov
	WHERE  ( fec_emi >='2014-12-04' AND fec_emi <='$fec_fin' )  AND tipo_mov = 4 AND nivel = 5 AND estado_mov = 1 AND cod_centro = 1";
	$db_ver->query($sql);
	if ($db_ver->next_row()){ 
	...
	INNER JOIN cuenta ON cuenta.cod_cuenta = d_movimientos.cuenta_dmov
	INNER JOIN centro_costos ON centro_costos.cod_centro = d_movimientos.centro_dmov
	WHERE  ( fec_emi >= '$fec_ini' AND fec_emi <= '$fec_fin' ) AND tipo_mov = 10 AND nivel = 5 AND estado_mov = 1 AND cod_centro = 1";
	$dbs->query($sqls);
	if ($dbs->next_row()){ 
	...
	INNER JOIN cuenta ON cuenta.cod_cuenta = d_movimientos.cuenta_dmov
	INNER JOIN centro_costos ON centro_costos.cod_centro = d_movimientos.centro_dmov
	WHERE  ( fec_emi >= '2014-12-04' AND fec_emi <= '$fec_fin' ) AND tipo_mov = 10 AND nivel = 5 AND estado_mov = 1 AND cod_centro = 1";
	$dbs->query($sqls);
	if ($dbs->next_row()){ 
