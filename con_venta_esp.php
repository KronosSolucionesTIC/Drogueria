...
if($eliminacion==1) {//confirmacion de insercion  
	$error=eliminar("m_factura_esp",$eli_codigo,"cod_fac");
...
/*$where.=" where cod_fac>0 and (  ";

echo $sql="select distinct punto_venta.cod_bod as cod_bodega from punto_venta inner join bodega on punto_venta.cod_bod=bodega.cod_bod where cod_ven=$global[2]";
exit;
...
{
	if($det==0) {
		$where.=" m_factura.cod_bod= $db->cod_bodega ";
		$det=1;
	}
	else 
		$where.=" or m_factura.cod_bod= $db->cod_bodega ";	
		
	$where.=" )";	
}

else 
	$where.=" where cod_fac>0  ";
...
$sql="select * , bodega1.nom_bod as nom_cliente from m_factura_esp left join usuario on usuario.cod_usu=m_factura_esp.cod_usu
left join bodega1 on bodega1.cod_bod=m_factura_esp.cod_cli left join bodega on bodega.cod_bod =m_factura_esp.cod_bod $where ORDER BY cod_fac  DESC ";
//exit;
                    ...
                    <tr>
                      <td><table width="624" border="0"  cellpadding="0">
						...
						while($db->next_row()) {
						   ...
						   //impresion	
							echo "<td align='center'><img src='imagenes/mirar.png' width='16' height='16'  style=\"cursor:pointer\" onClick=\"imprimir_inf('ver_factura_v_esp.php',$db->cod_fac,'grande')\" /></td>";	
