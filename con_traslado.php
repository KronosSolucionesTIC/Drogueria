...
if(!empty($busquedas)) { #codigo para buscar 
	...
	/*$sql="SELECT * ,
	 ...
	 (SELECT nom_bod FROM bodega WHERE cod_bod=cod_bod_sal_tras) AS bodega_salida,
	 (SELECT nom_bod FROM bodega WHERE cod_bod=cod_bod_ent_tras) AS bodega_entrada 
	 
	 FROM m_traslado 
	 $where  ORDER BY cod_tras DESC ";*/
                    ...
                    <tr>
                      <td bgcolor="#D1D8DE"><table width="624" border="0" cellspacing="0" cellpadding="0">
                        <tr>
                          ...
                          <td width="19"> 
						  <? if ($insertar==1) {?>
					  	  <img src="imagenes/agregar.png" width="16" height="16"  alt="Nuevo Registro" style="cursor:pointer" onClick="location.href='man_d_traslado.php?codigo=0&editar=<?=$editar?>&insertar=<?=$insertar?>&eliminar=<?=$eliminar?>'"/>
					  	  <? } ?></td>
                    ...
                    <tr>
                      <td><table width="624" border="0"  cellpadding="0">
						...
						while($db->next_row()) {
							...
							echo "<td align='center'><img src='imagenes/mirar.png' width='16' height='16'    alt='Despacho' style=\"cursor:pointer\" onClick=\"imprimir_inf('inf_despacho.php',$db->cod_tras,'mediano')\" /></td>";	
