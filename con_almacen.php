...
if(!empty($busquedas)) { #codigo para buscar 
 ...
 FROM bodega
  ...
  left JOIN rsocial ON  rsocial.cod_rso=bodega.cod_rso_bod $where ";*/
  $sql="SELECT
...
FROM
  bodega
  INNER JOIN rsocial ON (bodega.cod_rso_bod = rsocial.cod_rso)$where ";
                    ...
                    <tr>
                      <td bgcolor="#D1D8DE"><table width="624" border="0" cellspacing="0" cellpadding="0">
                        <tr>
                          ...
                          <td width="19"> 
						  <? if ($insertar==1) {?>
					  	  <img src="imagenes/agregar.png" width="16" height="16"  alt="Nuevo Registro" style="cursor:pointer" onClick="location.href='man_almacen.php?codigo=0&editar=<?=$editar?>&insertar=<?=$insertar?>&eliminar=<?=$eliminar?>'"/>
					  	  <? } ?></td>
