 ...
 <link href="informes/styles1.css" rel="stylesheet" type="text/css" />	
		<TABLE width="100%" border="1">
			...
			<INPUT type="hidden" name="id" value="<?=$id?>">
			  	...
			  	<tr>
			  		<td><div align="center" class="Estilo2">SEMAFORO <? echo date('d-M-Y');?></div></td>
			    </tr>
				<?
				$total=0;
				$sql = "SELECT cant_ref_kar,cod_bod_kar,cod_mar,nom_mar FROM kardex	
					...
					$where_bodegas AND ((cod_mar = 21)OR(cod_mar = 1027)OR(cod_mar = 1025)OR(cod_mar = 1032)OR(cod_mar = 1031)OR(cod_mar = 1030)) 
					...
					while($db->next_row()){ 
						echo "<br>\n";	
			  			echo "<table border='1'>";
						echo "<tr>";
						echo "<td width='300' class='Estilo2'>";
						echo $db->nom_mar; 
						echo "</td>";
						$dbt = new Database();
						if(($db->cod_mar == 30)or($db->cod_mar == 1021)or($db->cod_mar == 1024)or($db->cod_mar == 1025)){
							$orden = 'nom_pes';
						}
						else{
							$orden = 'cod_talla';
						...
						echo "</tr>";
							...
							while($dbr->next_row()){
								...
								while($dbc->next_row()){
									...
									if($dbsum->cantidad == 0){
										echo "<img src='imagenes/error.png' alt='Buscar' width='16' height='16' style='cursor:pointer'  onClick='buscar()'/>";
									}
									else if(($dbsum->cantidad > 0)and($dbsum->cantidad <= 10)){
										echo "<img src='imagenes/alerta.png' alt='Buscar' width='16' height='16' style='cursor:pointer'  onClick='buscar()'/>";
									}
									else if($dbsum->cantidad > 0){
										echo "<img src='imagenes/ok.png' alt='Buscar' width='16' height='16' style='cursor:pointer'  onClick='buscar()'/>";
									}
									echo "</td>";
								}
								echo "</tr>";
